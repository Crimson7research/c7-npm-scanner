...
<head>
    ...
    <style>
        * {
        ...
        .clean-packages-table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 15px;
        }

        .clean-packages-table th,
        .clean-packages-table td {
            text-align: left;
            padding: 12px 15px;
            border-bottom: 1px solid #e9ecef;
        }
...
"@
    ...
    # Add Clean Packages section
    if ($totalClean -gt 0) {
        $html += @"
            <div class="finding-section">
                <h3>Clean Packages ($totalClean)</h3>
"@
        if ($totalClean -gt 10) {
            $html += @"
                <div class="collapsible-section">
                    <button class="toggle-btn" onclick="toggleCleanPackages()">Show All Clean Packages</button>
                    <div id="cleanPackagesTable" class="hidden">
"@
        }

        $html += @"
                        <table class="clean-packages-table">
                            <thead>
                                <tr>
                                    <th><span class="clean-icon">[✓]</span>Package Name</th>
                                    <th>Version</th>
                                    <th>Status</th>
                                </tr>
                            </thead>
                            <tbody>
"@
        foreach ($package in $scanData.CleanPackages) {
            $html += @"
                                <tr>
                                    <td>$($package.Name)</td>
                                    <td>$($package.Version)</td>
                                    <td><span style="color: #28a745;">[✓] Verified Clean</span></td>
                                </tr>
"@
        }
        ...
        if ($totalClean -gt 10) {
            $html += @"
                    ...
                    <div id="cleanPackagesSummary">
                        <p style="color: #28a745; font-weight: bold;">
                            All $totalClean packages scanned and verified clean.
                            <a href="#" onclick="toggleCleanPackages(); return false;" style="color: #28a745;">Click to view full list</a>
                        </p>
                    </div>
                </div>
"@
        ...
        $html += @"
            <div class="finding-section">
                ...
                <div class="no-findings">No clean packages found (packages may be malicious, suspicious, or require manual check)</div>
            </div>
"@
    }
    ...
    $html += @"
        ...
        <footer class="footer">
            <div>Crimson7 NPM Security Scanner v1.2.0 | <a href="https://crimson7.io" style="color: #ffc107; text-decoration: none;">crimson7.io</a></div>
            <div class="timestamp">Report generated on $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') | Shai Hulud Update</div>
        </footer>
    </div>

    <script>
        function toggleCleanPackages() {
            const table = document.getElementById('cleanPackagesTable');
            const summary = document.getElementById('cleanPackagesSummary');
            const button = document.querySelector('.toggle-btn');

            if (table.classList.contains('hidden')) {
                table.classList.remove('hidden');
                summary.classList.add('hidden');
                button.textContent = 'Hide Clean Packages';
            } else {
                table.classList.add('hidden');
                summary.classList.remove('hidden');
                button.textContent = 'Show All Clean Packages';
            }
        }
    </script>
</body>
