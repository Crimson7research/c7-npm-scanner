                            ...
                            if ($version.folder) {
                                ...
                                if ($malCheck) {
                                    ...
                                    $results.MaliciousPackagesFound += @{
                                        Name = $packageName
                                        Version = $versionNum
                                        Path = "$RepoUrl$packageName/$versionNum/"
                                        Status = "MALICIOUS"
                    ...
                    if ($maliciousVersionsToCheck.Count -gt 0) {
                        ...
                        if (-not $foundAny) {
                            ...
                            $results.CleanPackages += @{
                                ...
                                Path = "$RepoUrl$packageName/$latestVersion/"
                                Status = "CLEAN"
                                    ...
                                    if (Test-Path $packageDir) {
                                        ...
                                        if (-not $foundSuspicious) {
                                            ...
                                            $results.CleanPackages += @{
                                                Name = $package.name
                                                Version = $latestVersion
                                                Path = "$RepoUrl$($package.name)/$latestVersion/"
                                                Status = "CLEAN"
    ...
    foreach ($packageFile in $packageFiles) {
        ...
        switch ($result.Status) {
            ...
            "CLEAN" {
                ...
                Write-ColorOutput "  [CLEAN] Package verified safe" -Level "Clean"
            }
        }
        $results.TotalPackagesScanned++
    }
...
if ($results.CleanPackages.Count -gt 0) {
    $sampleCleanPackages = $results.CleanPackages | Select-Object -First 5 | ForEach-Object { "$($_.Name)@$($_.Version)" }
    if ($results.CleanPackages.Count -le 5) {
        Write-ColorOutput "  Clean: $($sampleCleanPackages -join ', ')" -Level "Clean"
    } else {
        Write-ColorOutput "  Sample: $($sampleCleanPackages -join ', ') (and $($results.CleanPackages.Count - 5) more)" -Level "Clean"
    }
}
